...
#define AHT21B_SENSOR_ADDR               0x38        /*! */
#define AHT21B_STATUS_REG_ADDR           0x71        /*!< Register addresses of the "who am I" register */
...
void app_main(void)
{
    ...
    i2c_master_write_byte(cmd, ((AHT21B_SENSOR_ADDR)<<1) | I2C_MASTER_WRITE, true);
    i2c_master_write_byte(cmd, (AHT21B_STATUS_REG_ADDR<<1), true);
    i2c_master_stop(cmd);

     esp_err_t espRc = i2c_master_cmd_begin(I2C_NUM, cmd, 10/portTICK_PERIOD_MS);
    if (espRc == ESP_OK) {
        ESP_LOGI(TAG, "Status Cmd Sent");
    } else {
        ESP_LOGE(TAG, "Failed. code: 0x%.2X", espRc);
    }
    ...
    while(1)
    {
        ...
        if (espRc == ESP_OK) {
            ESP_LOGI(TAG, "Initialized Registers");
        } else {
            ESP_LOGE(TAG, "Failed. code: 0x%.2X", espRc);
        }
        i2c_cmd_link_delete(cmd);
        ...
        cmd= i2c_cmd_link_create();
        i2c_master_start(cmd);
        i2c_master_write_byte(cmd, ((AHT21B_SENSOR_ADDR)<<1) | I2C_MASTER_READ, true);
        i2c_master_read_byte(cmd, data, true);
        i2c_master_stop(cmd);

        espRc = i2c_master_cmd_begin(I2C_NUM, cmd, 10/portTICK_PERIOD_MS);
        if (espRc == ESP_OK) {
            ESP_LOGI(TAG, "Status Code %d", data[0]);
        } else {
            ESP_LOGE(TAG, "Failed. code: 0x%.2X", espRc);
        }
        i2c_cmd_link_delete(cmd);
        ...
        // i2c_master_start(cmd);
        // i2c_master_write_byte(cmd, ((AHT21B_SENSOR_ADDR)<<1) | I2C_MASTER_READ, true);
        // i2c_master_read_byte(cmd, temp[0], true);
        // i2c_master_read_byte(cmd, temp +1, true);
        // i2c_master_read_byte(cmd, temp+2, true);
        //         i2c_master_read_byte(cmd, temp+3, true);
        //                 i2c_master_read_byte(cmd, temp+4, true);
        // i2c_master_stop(cmd);

        
        espRc = i2c_master_read_from_device(0,AHT21B_SENSOR_ADDR, temp, 6, 10/portTICK_PERIOD_MS);
        if (espRc == ESP_OK) {
            // int i =0;
            // for(i = 0;i<6;i++){
            //     ESP_LOGI(TAG, "Temperature[%d] %d",i, temp[i]);
            //     }
            ...
            indoor_temp = ((float)t / 0x100000) * 200 - 50;

             ESP_LOGI(TAG, "Temperature %f", indoor_temp);
        } else {
            ESP_LOGE(TAG, "Failed. code: 0x%.2X", espRc);
        }
        //i2c_cmd_link_delete(cmd);
